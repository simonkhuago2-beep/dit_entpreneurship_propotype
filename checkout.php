   ...
   $arrayOfOrderItems = array();
        ...
        /*foreach ($_SESSION['cart'] as $value){
            ...
            if ($value['service']=='transfer') {
               $transactionDetails = '{"id":"'.$value['serviceId'].'","name":"'.$fullname.'","tel":"'.$value['phone'].'","email":"'.$value['email'].'","pick_addr":"'.$value['pick_loc'].'","pick_date":"'.$value['pick_date'].'","pick_time":"'.$value['pick_time'].'","drop_addr":"'.$value['drop_loc'].'","drop_date":"'.$value['drop_date'].'","drop_time":"'.$value['drop_time'].'"}';

               //$toCedis = currencyConverter02($value['subtotal'],'USD','GHS');

               $makeBooking = $conn->insert('booking',array('trans_id'=>$transactionId,'trans_type'=>'transfers','trans_details'=>$transactionDetails,'trans_cost'=>$value['subtotal'],'payment_mode'=>$mode,'made_by'=>$agent) );
            }elseif ($value['service']=='cruise') {
                $transactionDetails = '{"id":"'.$value['serviceId'].'","name":"'.$fullname.'","tel":"'.$value['phone'].'","email":"'.$value['email'].'","date":"'.$value['date'].'","time":""}';

                //$toCedis = currencyConverter02($value['subtotal'],'USD','GHS');

                $makeBooking = $conn->insert('booking',array('trans_id'=>$transactionId,'trans_type'=>'excursions','trans_details'=>$transactionDetails,'trans_cost'=>$value['subtotal'],'payment_mode'=>$mode,'made_by'=>$agent) );
            }elseif ($value['service']=='package') {
                $transactionDetails = '{"id":"'.$value['serviceId'].'","name":"'.$fullname.'","tel":"'.$value['phone'].'","email":"'.$value['email'].'"}';
                $makeBooking = $conn->insert('booking',array('trans_id'=>$transactionId,'trans_type'=>'tours','trans_details'=>$transactionDetails,'trans_cost'=>$value['subtotal'],'payment_mode'=>$mode,'made_by'=>$agent) );

                //$toCedis = currencyConverter02($value['subtotal'],'USD','GHS');

            }elseif ($value['service']=='events') {
                $transactionDetails = '{"id":"'.$value['serviceId'].'","name":"'.$fullname.'","tel":"'.$value['phone'].'","email":"'.$value['email'].'"}';
                $makeBooking = $conn->insert('booking',array('trans_id'=>$transactionId,'trans_type'=>'events','trans_details'=>$transactionDetails,'trans_cost'=>$value['subtotal'],'payment_mode'=>$mode,'made_by'=>$agent) );

                //$toCedis = currencyConverter02($value['subtotal'],'USD','GHS');

            }elseif ($value['service']=='hotel') {
                $transactionDetails = '{"roomId":"'.$value['roomId'].'","hotelId":"'.$value['serviceId'].'","name":"'.$fullname.'","tel":"'.$value['phone'].'","email":"'.$value['email'].'","city":"","zip":"","request":"","breakfast":"","rollaway":"","extrabed":"","adult":"'.$value['adult'].'","child":"'.$value['kids'].'","checkin":"'.$value['checkin'].'","checkout":"'.$value['checkout'].'","days":"'.$value['duration'].'","roomqty":"'.$value['roomqty'].'"}';

                //$toCedis = currencyConverter02($value['subtotal'],'USD','GHS');
                //$toCedis = currencyConverter('USD','GHS',$value['subtotal']);

                $makeBooking = $conn->insert('booking',array('trans_id'=>$transactionId,'trans_type'=>'hotels','trans_details'=>$transactionDetails,'trans_cost'=>$value['subtotal'],'payment_mode'=>$mode,'made_by'=>$agent) );
     
            }
            
            //echo $value['subtotal'].'<br>';
            //echo $toCedis;
            /*-------------------prepare with booking---------------replace to title----*/
           /*$subTotalAmount += $toCedis;//$value['subtotal']*$toCedis;
            $arrayOfOrderItems[] = $iwl->buildOrderItem(count_id(), $value['service'], $subTotalAmount, 1, $subTotalAmount);*/
        ...
        //$shipping_cost=0;
        //$tax_amount=0;
        //$Total= $subTotalAmount + $shipping_cost + $tax_amount;
        //$response = $paystack->ProcessPaymentOrder($transactionId, $subTotalAmount, $shipping_cost, $tax_amount, $Total, $comment1, $comment2, $arrayOfOrderItems);
        //$redirectUrl = $paylive.$response->ProcessPaymentOrderResult;
...
if ($mode == 'reserve'){
        ...
        foreach ($_SESSION['cart'] as $value){
            ...
            if ($value['service']=='transfer') {
               ...
               $info = find_by_id('transfers',$transferId);
               $transactionDetails = '{"id":"'.$value['serviceId'].'","name":"'.$fullname.'","tel":"'.$value['phone'].'","email":"'.$value['email'].'","pick_addr":"'.$value['pick_loc'].'","pick_date":"'.$value['pick_date'].'","pick_time":"'.$value['pick_time'].'","drop_addr":"'.$value['drop_loc'].'","drop_date":"'.$value['drop_date'].'","drop_time":"'.$value['drop_time'].'"}';
               $makeBooking = $conn->insert('booking',array('trans_id'=>$transactionId,'trans_type'=>'transfers','trans_details'=>$transactionDetails,'trans_cost'=>$value['subtotal']) );
            
               if ($makeBooking) {
                    /*---------create notification-------------*/
                    $conn->insert('notify',array('type'=>'Booking','tag'=>'transfers','comment'=>'New Transfer Reservation.'));
                    }
                ...
                if ($makeBooking) {
                    /*---------create notification-------------*/
                    $conn->insert('notify',array('type'=>'Booking','tag'=>'excursions','comment'=>'New Excursion Reservation.'));
                }
                ...
                if ($makeBooking) {
                    //create notification
                    $conn->insert('notify',array('type'=>'Booking','tag'=>'tours','comment'=>'New Tour Reservation.'));
                }
                ...
                if ($makeBooking) {
                    //create notification
                    $conn->insert('notify',array('type'=>'Booking','tag'=>'events','comment'=>'New Events Reservation.'));
                }
                ...
                if ($makeBooking) {
                //create notification
                $conn->insert('notify',array('type'=>'Booking','tag'=>'hotels','comment'=>'New ApartHotel Reservation.'));
            }
        }
        ...
        unset($_SESSION['cart']);
        Session::flash('success','Transaction completed');
        Redirect::to('booking-voucher-reservation.php');
    }
}
...
if ($mode == 'paystack'){
    ...
    foreach ($_SESSION['cart'] as $value){
        ...
        if ($value['service']=='transfer') {
           ...
           $transactionDetails = '{"id":"'.$value['serviceId'].'","name":"'.$fullname.'","tel":"'.$value['phone'].'","email":"'.$value['email'].'","pick_addr":"'.$value['pick_loc'].'","pick_date":"'.$value['pick_date'].'","pick_time":"'.$value['pick_time'].'","drop_addr":"'.$value['drop_loc'].'","drop_date":"'.$value['drop_date'].'","drop_time":"'.$value['drop_time'].'"}';
           $recepit_id = substr(GUID(), 9,9);
           $makeBooking = $conn->insert('booking',array('trans_id'=>$transactionId,'trans_type'=>'transfers','trans_details'=>$transactionDetails,'trans_cost'=>$value['subtotal'],'payment_status'=>1,'booking_no'=>$recepit_id,'payment_mode'=>$mode,'made_by'=>$paystack) );
           ...
           if ($makeBooking) {
            ...
            $transactionDetails = '{"id":"'.$value['serviceId'].'","name":"'.$fullname.'","tel":"'.$value['phone'].'","email":"'.$value['email'].'","date":"'.$value['date'].'","time":""}';
            $recepit_id = substr(GUID(), 9,9);
            $makeBooking = $conn->insert('booking',array('trans_id'=>$transactionId,'trans_type'=>'excursions','trans_details'=>$transactionDetails,'trans_cost'=>$value['subtotal'],'payment_status'=>1,'booking_no'=>$recepit_id,'payment_mode'=>$mode,'made_by'=>$paystack) );
            ...
            $transactionDetails = '{"id":"'.$value['serviceId'].'","name":"'.$fullname.'","tel":"'.$value['phone'].'","email":"'.$value['email'].'"}';
            $recepit_id = substr(GUID(), 9,9);
            $makeBooking = $conn->insert('booking',array('trans_id'=>$transactionId,'trans_type'=>'tours','trans_details'=>$transactionDetails,'trans_cost'=>$value['subtotal'],'payment_status'=>1,'booking_no'=>$recepit_id,'payment_mode'=>$mode,'made_by'=>$paystack) );
            if ($makeBooking) {
                //create notification
                $conn->insert('notify',array('type'=>'Booking','tag'=>'tours','comment'=>'New Tour Reservation.'));
            ...
            // insert booking
                $makeBooking = $conn->insert('booking',array('trans_id'=>$transactionId,'trans_type'=>'hotels','trans_details'=>$transactionDetails,'trans_cost'=>$value['subtotal'],'payment_status'=>1,'payment_mode'=>$mode,'booking_no'=>$recepit_id,'made_by'=>$paystack) );
